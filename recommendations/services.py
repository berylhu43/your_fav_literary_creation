...
from catalog.clients import _google_books_get, _tmdb_get
from catalog.models import Catalog, Genre
...
def _build_extract_prompt(query, media_types):
    ...
    genre_names = list(
        Genre.objects.filter(catalogs__media_type__in=media_types)
        .distinct()
        .order_by("name")
...
def _sample_reviews(user, filters, media_types, per_bucket=10):
    ...
    qs = qs.select_related("catalog").prefetch_related("catalog__credits").distinct()
    ...
    print(f">>> sampled {len(result)} reviews: {[r.catalog.title for r in result]}")
    return result
...
def _build_recommend_prompt(query, media_types, samples, filters):
    ...
    if genres:
        constraints.append(
            f"- Genre: every recommendation MUST match at least one of: {', '.join(genres)}."
        )
    year_min = filters.get("year_min")
    year_max = filters.get("year_max")
    if year_min and year_max:
        constraints.append(
            f"- Release year: MUST be between {year_min} and {year_max} (inclusive)."
        )
    elif year_min:
    ...
    if artist:
        constraints.append(
            f"- Prefer works involving {artist} when possible (soft preference, not mandatory)."
        )
...
def _parse(raw):
    """
    Defensive parsing of LLM output. Returns a list of recommendations, or [] if parsing fails.
    """
    try:
        data = json.loads(raw)
    except (json.JSONDecodeError, TypeError):
...
def _filter_seen(user, recommendations, media_types):
    seen = set(
        _norm_title(t)
        for t in Review.objects.filter(
            user=user,
            catalog__media_type__in=media_types,
        ).values_list("catalog__title", flat=True)
    )
    return [r for r in recommendations if _norm_title(r.get("title", "")) not in seen]
