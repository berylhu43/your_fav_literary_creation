...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--out', default='match_report.csv',
                            help='output CSV path for the match stage')
        parser.add_argument('--user', help='username to attach reviews to (commit stage)')
    ...
    def _match(self, infile, outfile):
        ...
        fieldnames = ['douban_id', 'title_zh', 'title_original', 'rating',
                    'watched_date', 'tmdb_id', 'tmdb_media_type', 'tmdb_title',
                    'status', 'candidates']
        ...
        with open(infile, encoding='utf-8-sig') as f:
            for row in csv.DictReader(f):
                ...
                _, was_created = Review.objects.update_or_create(
                    user=user, catalog=work, defaults=defaults,
                )
                created += was_created
                self.stdout.write(f"  {'created' if was_created else 'updated'}: "
