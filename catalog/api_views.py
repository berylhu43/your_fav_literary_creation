...
from .serializers import ArtistSerializer, CatalogSerializer
from .services import (
    _slim_movie_result,
    _slim_tv_result,
    get_artist_filmography,
    get_cached_movie_genres,
    get_cached_tv_genres,
    get_or_create_work,
    get_popular_movies,
    get_popular_tv,
    search_external,
...
class CatalogReviewsAPI(APIView):
    ...
    ]  # login to show my review, otherwise null

    def get(self, request, pk):
        reviews = (
            Review.objects.filter(catalog_id=pk)
            .select_related("user")
            .order_by("-created_at")
        )
        ...
        return Response(
            {
                "my_review": PublicReviewSerializer(my_review).data
                if my_review
                else None,
