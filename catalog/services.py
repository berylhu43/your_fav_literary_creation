...
from . import clients
from .models import Artist, Catalog, Credit, Genre
...
def _add_book_credits(work, author_names):
    for name in author_names:
        artist, _ = Artist.objects.get_or_create(
            name=name, defaults={"source": Catalog.Source.GOOGLE_BOOKS}
        )
...
def _get_or_create_artists(person):
    ...
    profile_url = (
        f"https://image.tmdb.org/t/p/w185{profile_path}" if profile_path else ""
    )
    artist, _ = Artist.objects.get_or_create(
        source=Catalog.Source.TMDB,
...
def _slim_book_result(item):
    ...
    return {
        ...
        "poster_url": image_links.get("thumbnail", "").replace(
            "http://", "https://", 1
        ),
    }
